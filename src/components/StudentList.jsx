    ...
    <Modal visible={visible} animationType="slide">
      <View style={styles.modalStyle}>
        ...
        <View style={styles.wrapper}>
          ...
          <TextInput placeholder='Enter student name' style={styles.textInput} value={name} onChangeText={setName} />
        </View>
        <View style={styles.buttonWrapper}>
          <Pressable style={styles.buttonCancel} onPress={onClose}>
            <Text style={styles.buttonText}>Cancel</Text>
          </Pressable>
          <Pressable style={styles.buttonSubmit} onPress={() => updateBoot(updateIndex, {id: Number(id), name: name})}>
...
export default function StudentList() {
  ...
  // To delete a student
  const deleteStudent = async (item_index) => {
    try {
      let response = await fetch(`${BASE_URL}/students/${item_index}`, {
        method: 'DELETE',
      });
      ...
      if (!response.ok) {
        ...
        setStudentList(updatedBootList);
      }
    } catch (error) {
  ...
  const handleUpdate=(index)=>{
    ...
    setBootToUpdate(studentList[index]);
    ...
    <View style={styles.container}>
      ...
      <Modal visible={modalVisible} animationType="slide">
        <View style={styles.modalStyle}>
          ...
          <View style={styles.wrapper}>
            <Text style={styles.inputLabel}>ID:</Text>
            <TextInput placeholder='Enter student ID...' style={styles.textInput} value={value1} onChangeText={setValue1}/>
          </View>
          <View style={styles.wrapper}>
            <Text style={styles.inputLabel}>Name:</Text>
            <TextInput placeholder='Enter student name...' style={styles.textInput} value={value2} onChangeText={setValue2}/>
          </View>
          <View style={styles.buttonWrapper}>
      ...
      <ScrollView style={styles.list}>
        {studentList.map((item, index)=>(
          <Pressable key={index} onLongPress={()=>deleteStudent(index)} onPress={()=>handleUpdate(index)}>
